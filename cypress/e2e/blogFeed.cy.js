describe("Blog Feed Page", () => {
  ...
  describe("Carousel", () => {
    ...
    it("should navigate through carousel using next button", () => {
      const firstPostTitle = cy
        .contains(".carousel-item.active h2")
        .invoke("text");
      cy.contains("Next").click();
      cy.get(".carousel-item.active h2").should(
        "not.have.text",
        firstPostTitle
      );
    });
    ...
    it("should have clickable read more buttons", () => {
      cy.get(".carousel-item.active").find("a").contains("Read more").click();
  ...
  describe("Blog Post Grid", () => {
    ...
    it("should have clickable post thumbnails", () => {
      cy.get(".blog-grid .post-card").first().click();
      cy.url().should("include", "/post/index.html");
    });
